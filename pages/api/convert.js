...
export default async function handler(req, res) {
  if (req.method === 'POST') {
    try {
      ...
      convertapi.upload(files.file1.filepath, files.file1.originalFilename).then(function(file1Result) {
        const file1Url = `https://v2.convertapi.com/d/${file1Result.fileId}`;
        console.log("Done uploading");
        convertapi.upload(files.file2.filepath, files.file2.originalFilename).then(function(file2Result) {
          ...
          convertapi.convert('compare', {
            ...
            if (result.response.Files && result.response.Files[0] && result.response.Files[0].FileName) {
              ...
              result.saveFiles('./temp/').then(() => {  // Notice the change here
